...
private val CMD_WRITE_CONFIG: Byte = 0x04 // Command to write configuration (requires a 96-byte payload)
private val CMD_FACTORY_RESET: Byte = 0x0A
private val CMD_GET_SYSTEM_INFO: Byte = 0x2B.toByte()

// MIME Type for sending the command to the TAG (First Scan)
private const val MIME_COMMAND_TYPE = "application/x-cmd"
// MIME Type for waiting for the data response from the TAG (Second Scan)
...
class MainActivity : AppCompatActivity(), NfcInteractionListener {
    ...
    override fun requestNextCommand(commandId: Byte) {
        ...
        sharedViewModel.setConfigDataToWrite(null) // Clear any pending write data
    ...
    override fun requestWriteConfig() {
        ...
        sharedViewModel.sendCommand(CMD_WRITE_CONFIG) // Set 0x04
    ...
    private fun setupViewModelObservers() {
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                // This observer converts ViewModel emissions into Toasts.
                launch {
    ...
    /**
     * Enables foreground dispatch to prioritize NDEF and other technology detection.
     */
    ...
    /**
     * Logs all detected technologies in the TAG.
     * @param tag The detected tag.
     */
    private fun logTagTechnologies(tag: Tag) {
        val techList = tag.techList.joinToString(", ")
        lifecycleScope.launch {
            LogManager.log("TAG DIAGNOSTICS: ID=${tag.id.toHexString()}, Detected Techs=[$techList]")
        }
    }
     ...
     * **Contains the global try/catch block to prevent unexpected app closures.**
     * @param tag The detected tag.
     */
    private suspend fun processNdefTransaction(tag: Tag) = withContext(Dispatchers.IO) {
        // Global try/catch block to catch any propagated exceptions,
        // ensuring the coroutine doesn't fail and close the app.
        try {
            val commandId = sharedViewModel.pendingCommand.value
            val ndef = Ndef.get(tag)
            val ndefFormatable = NdefFormatable.get(tag)
            val nfcV = NfcV.get(tag) // Get the NfcV object for the fallback
            ...
            if (commandId != null) {
                ...
                // 1. Try standard NDEF write (TAG is already valid and writable NDEF)
                if (ndef != null) {
                    success = executeNdefWriteCommand(ndef, commandId)
                }

                // 2. Try NDEF format and write (TAG is formattable, but the CC might be corrupt)
                if (!success && ndefFormatable != null) {
                    // If this fails (returns false), we move to the advanced fallback (step 3)
                ...
                if (!success && nfcV != null) {
                    lifecycleScope.launch { LogManager.log("Activating Advanced Fallback: Low-level CC formatting via NfcV.") }
                    ...
                    if (formatSuccess) {
                        ...
                        // EMIT SUCCESSFUL FALLBACK TOAST
                        sharedViewModel.emitWriteStatus("NfcV FALLBACK OK: TAG formatted. Scan again to write.")
                    } else {
                        ...
                        // EMIT FAILED FALLBACK TOAST
                        sharedViewModel.emitWriteStatus("NfcV ERROR: Failed to force format. Locked or unsupported.")
                    }
                    // Exit after the forced format attempt (a new scan is required for NDEF writing)
                    return@withContext
                }
                ...
                if (!success) {
                    sharedViewModel.setUiMessage("ERROR: Failed to process the command. TAG not NDEF and without support for forced formatting.")
                }
                return@withContext
            }
            ...
            if (ndef != null) {
                try {
                    ...
                    // This try-catch already handles specific 'I/O ERROR' from NDEF reading.
                    val errorMsg = "NDEF read error (I/O): ${e.message}"
                    lifecycleScope.launch { LogManager.log(errorMsg) }
                    sharedViewModel.setUiMessage(errorMsg)
                    // sharedViewModel.emitWriteStatus("NDEF I/O ERROR: Failed to read response.") // Toast for I/O
                } finally {
                    if (ndef.isConnected) {
                ...
                sharedViewModel.setUiMessage("Warning: Invalid/unformatted TAG. Ready to receive a command.")
            }

        } catch (e: Exception) {
            // ** GLOBAL CATCH TO PREVENT APP CLOSURE **
            val errorMsg = "CRITICAL unhandled Coroutine/IO Error: ${e.message}"
            ...
            sharedViewModel.emitWriteStatus("CRITICAL ERROR: Unhandled general failure.")
        }
    }

    /**
     * **Helper:** Creates the complete NDEF command payload, including the 96 bytes of
     * configuration if the command is CMD_WRITE_CONFIG (0x04).
     *
     * @param commandId The ID of the command to create.
     * @return The complete command payload as a ByteArray, or null if configuration data is missing.
     */
    private fun createNdefCommandPayload(commandId: Byte): ByteArray? {
        return if (commandId == CMD_WRITE_CONFIG) {
            ...
            if (configDataBytes == null || configDataBytes.size != 96) {
                ...
                sharedViewModel.emitWriteStatus("ERROR: Configuration data (96B) not available to write.")
                null
            } else {
    ...
    private fun executeNfcVForceFormatOnlyFallback(nfcV: NfcV): Boolean {
        ...
        val flags: Byte = 0x02 // Flags: Single addressing (no UID), Data Rate High
        ...
        try {
            ...
            // ISO 15693 response validation (empty or status byte 0x00 indicates success)
            ...
            val errorMsg = "NfcV Error (ISO 15693) when forcing CC format: ${e.message}. The chip might be completely locked."
            lifecycleScope.launch { LogManager.log(errorMsg) }
            sharedViewModel.setUiMessage(errorMsg)
            // EMIT NfcV ERROR TOAST
            sharedViewModel.emitWriteStatus("NfcV ERROR: I/O failure. The chip might be completely locked.")
            return false
        } finally {
            if (nfcV.isConnected) {
    ...
    /**
     * **First Scan (High-Level Forced):** Formats the TAG and writes the NDEF command message.
     * (Only used if NdefFormatable is available)
     * @param ndefFormatable The NdefFormatable object.
     * @param commandId The ID of the command to write.
     * @return True if the format and write were successful, false otherwise.
     */
    private fun executeNdefFormatAndWriteCommand(ndefFormatable: NdefFormatable, commandId: Byte): Boolean {

        // --- PREPARE NDEF PAYLOAD (Using the helper function) ---
        val fullPayload = createNdefCommandPayload(commandId)
        if (fullPayload == null) {
            // The helper has already emitted the Toast/UI Message for missing data.
            return false
        }
        ...
        try {
            ...
            // Write successful
            sharedViewModel.setUiMessage("First HIGH-LEVEL FORCED Scan OK: Formatted and $logMessage sent. Bring the TAG close again for the Second Scan!")
            // EMIT FORCED WRITE SUCCESSFUL TOAST
            ...
            val errorMsg = "NDEF error on format/write: ${e.message}. Resorting to NfcV."
            lifecycleScope.launch { LogManager.log(errorMsg) }
            // EMIT ERROR TOAST
    ...
    private fun executeNdefWriteCommand(ndef: Ndef, commandId: Byte): Boolean {
        ...
        try {
            ...
            if (!ndef.isWritable) {
                val errorMsg = "ERROR: NDEF TAG is not writable. Check the TAG lock."
                lifecycleScope.launch { LogManager.log(errorMsg) }
                sharedViewModel.setUiMessage(errorMsg)
                // EMIT LOCK ERROR TOAST
                sharedViewModel.emitWriteStatus("ERROR: The TAG is locked. Cannot write the command.")
                return false
            }
            ...
            // --- CATCH BLOCK FOR CONNECTION OR SECURITY ISSUES ---
        } catch (e: SecurityException) {
            // Catch: java.lang.SecurityException: Tag is out of date.
            val errorMsg = "Security Error (TAG Lost): The TAG has been moved or disconnected. Bring the TAG closer to retry."
            lifecycleScope.launch { LogManager.log("NFC_WRITE SecurityException (Stale Tag): ${e.message}") }
            sharedViewModel.setUiMessage(errorMsg)
            // EMIT CONNECTION ERROR TOAST
            ...
            val errorMsg = "Connection Error (I/O): The TAG moved or communication failed during the operation. Bring the TAG closer to retry."
    ...
    private fun executeNdefReadResponse(ndef: Ndef) {
        ...
        try {
                ...
                if (record.tnf == NdefRecord.TNF_MIME_MEDIA) {
                    ...
                    if (recordType == MIME_RESPONSE_TYPE) {
                        ...
                        if (payload.isNotEmpty()) {
                            ...
                            sharedViewModel.setUiMessage("Second Scan OK: Data from 0x${originalCommandId.toHexString()} received. Ready for a new command.")
                            responseFound = true
                            break
                        }
                        ...
                        // EMIT SYNCHRONIZATION ERROR TOAST
                        sharedViewModel.emitWriteStatus("SYNC ERROR: TAG still has the command. Rescan in 2s.")
                        responseFound = true
                        break
                    }
                }
            ...
            if (!responseFound) {
                ...
                // EMIT RESPONSE ERROR TOAST
                sharedViewModel.emitWriteStatus("NDEF ERROR: Response record not found.")
            }

        } catch (e: Exception) {
            val errorMsg = "NDEF read error (Logic): ${e.message}"
            lifecycleScope.launch { LogManager.log(errorMsg) }
            sharedViewModel.setUiMessage(errorMsg)
            // EMIT UNEXPECTED ERROR TOAST
